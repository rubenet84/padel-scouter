...
def get_records(
    ...
    record_config = [
        ("points", "Puntos FEP"), ("wins", "Victorias"), ("streak", "Racha"),
        ("tournaments_won", "Torneos Ganados"), ("finals", "Finales"),
        ("semis", "Semifinales"), ("sets_won", "Sets Ganados"),
    ...
    for metric_key, metric_label in record_config:
        ...
        for pid in sorted_pids:
            ...
            records.append(
                PlayerRecord(
                    player_id=pid, name=p.name, category=p.category,
                    value=m[metric_key], metric_key=metric_key,
...
def get_evolution(
    ...
    return [
        EvolutionEntry(
            player_id=p.id, name=p.name, category=p.category,
            current_points=fep_points.get(p.id, 0), sparkline=[],
        )
        for p in players
    ]
...
def get_community_highlights(
    ...
    if most_points_pid and most_points_val > 0:
        ...
        most_points = PlayerBrief(
            id=most_points_pid, name=p.name, category=p.category,
    ...
    if best_form_pid:
        ...
        best_form = PlayerBrief(
            id=best_form_pid, name=p.name, category=p.category,
            points=fep_points.get(best_form_pid, 0), win_pct=best_form_val,
        )
    ...
    for key, st in pair_stats.items():
        ...
        if wp > best_pair_win_pct:
            ...
            members = list(key)
            best_pair = {
                "player1_id": str(members[0]), "player2_id": str(members[1]),
                "player1_name": players_map[members[0]].name,
                "player2_name": players_map[members[1]].name,
                "win_pct": round(wp, 1), "matches": st["total"],
            }
    ...
    if most_active_pid and most_active_val > 0:
        ...
        most_active = PlayerBrief(
            id=most_active_pid, name=p.name, category=p.category,
            points=fep_points.get(most_active_pid, 0),
            win_pct=metrics.get(most_active_pid, {}).get("win_pct", 0.0),
        )

    return CommunityHighlights(
        most_points=most_points, best_form=best_form,
        best_pair=best_pair, most_active=most_active,
    )
